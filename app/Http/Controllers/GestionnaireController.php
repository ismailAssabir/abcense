...
class GestionnaireController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Pagination de la collection en mémoire (pour conserver les indicateurs dynamiques)
        $currentPage = \Illuminate\Pagination\LengthAwarePaginator::resolveCurrentPage() ?: 1;
        $perPage = 10;
        $currentItems = $stagiaires->slice(($currentPage - 1) * $perPage, $perPage)->values()->all();
        $paginatedStagiaires = new \Illuminate\Pagination\LengthAwarePaginator(
            $currentItems,
            $stagiaires->count(),
            $perPage,
            $currentPage,
        ...
        return view('gestionnaire.dashboard', [
            'groupes' => $groupes,
            'stagiaires' => $paginatedStagiaires,
            'totalStagiaires' => $totalStagiaires,
            'nbAlerteWarning' => $nbAlerteWarning,
            'nbAlerteCritical' => $nbAlerteCritical,
            'nbJustificationsAttente' => $nbJustificationsAttente,
    ...
    public function show(Request $request, Stagiaire $stagiaire)
    {
        ...
        return view('gestionnaire.show', [
            'stagiaire' => $stagiaire,
            'absences' => $absences,
            'totalAbsencesCount' => $totalAbsencesCount,
            'totalHoursJustified' => $totalHoursJustified,
            'totalHoursUnjustified' => $totalHoursUnjustified,
